...
if (p) {
    p.addEventListener('click', function () {
        const selElement = this.closest('.sel');/* 가장 가까이 있는 부모 선택 */
        if (selElement) {
            selElement.classList.toggle('on') /* 왔다갔다 */
        }
    });
}
...
visionCards.forEach((card,i)=>{
    const tl = gsap.timeline({
        scrollTrigger : {
            ...
            end : `top+=${(i+1)*300} center`, /* 끝나는 조건 : 300후에 */
            scrub : 1.2,
        }
    })
...
gsap.timeline({
    scrollTrigger : {
        ...
        scrub : 1, //스크롤을 움직이면 애니메이션도 같이 부드럽게 따라가게 만드는 속성
...
.to(cards,{y:-300,z:-200,skewY:5,duration:1,ease: "power2.in", stagger:1,},'+=0.5')//앞 애니메이션 끝나고 0.5 쉬었따가 등장
