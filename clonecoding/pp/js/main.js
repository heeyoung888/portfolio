...
$(function () {
    ...
    const pathLength = path.getTotalLength(); //경로의 총 길이 계산
    path.style.strokeDasharray = pathLength; //경로를 점선처럼 보이게 설정
    path.style.strokeDashoffset = pathLength; //경로를 숨기기 위해 대시 오프셋을 길이로 설정
    ...
    function scrollHandler() {
        ...
        //화면 높이 고려한 스크롤 위치 계산
        path.style.strokeDashoffset = calcDashOffset(scrollY, svgCon, pathLength)
    }

    window.addEventListener('scroll', scrollHandler); //스크롤 이벤트 리스너 추가
    ...
    gsap.timeline({
        scrollTrigger: {
            //스크롤을 감지할 대상 : 
            trigger: '#con3',
            start: '0% 80%',//con3의 맨 위(0%)가 화면에 80%지점에 닿을 때 애니시작
            end: '100% 100%',//con3의 맨 아래(100%)가 화면의 맨아래(100%)에 닿을 때 애니끝
            scrub: 1,
            //스크롤 움직임에 따라 애니메이션도 같이 움직이기 함
        }
    })
        .to('#con3', {
            backgroundColor: '#fff',
            color: '#000',
            duration: 5,
            ease: 'none' //가속도 , 애니메이션이 부드럽게 변하지 않고 스크롤에 딱 맞게 움직이게 설정
        }, 0)//처음에 시작됐을 때 만들어주는 것(바뀌는 상황만 넣어줌)
        .fromTo('#con3 .videoWrap video', { 'clip-path': 'inset(60% 60% 60% 60% round 30%)' }, { 'clip-path': 'inset(0% 0% 0% 0% round 0%)', duration: 5, ease: 'none' }, 0) // 시작과 끝을 넣어줌
       ...
       // 각 섹션에 애니메이션 적용
        ...
        animations.forEach((anim, index)=>{
            ani[index] = gsap.to(anim.target,{
                ...
                scrollTrigger:{
                    trigger : anim.target,
                    containerAnimation: scrollTween, // 가로 스크롤 애니메이션과 동기화(그 상황에서 나올 수 있게 해줌)
                    start:'left center',
        ...
        pageButtons.forEach((button,index)=>{
            button.addEventListener('click',()=>{
                ...
                //가로 스크롤 애니메이션을 직접 설정
                gsap.to(sections,{
                    xPercent : targetPosition, //해당 섹션으로 이동하기
                    duration:1, //이동시간
                    ease: "power2.inOut", // 부드러운 애니메이션
                    onComplete : ()=>{
                        triggerAnimation(index); //각 섹션에 대한 애니메이션 실행
                        updatePagination(index); //각 섹션에 대한 애니메이션 실행
                    }
                })
            })
        })
        ...
        con5Article.forEach((el,i)=>{
            gsap.timeline({
                scrollTrigger : {
                    ...
                    end : 'top 20%', //화면 위쪽으로 거의다 올라왔을 때 끝
                    scrub : 0.5 //스크롤에 따라 부드럽게 반응 /1 or 0.5
                }
            }).fromTo(
